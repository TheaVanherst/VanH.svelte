<script>
    ...
    import {navigationDirectories, directoryStatus}
...
<ImageFloatCard {hover} {active} {absolute}>
	...
	<div slot="alt">
		...
		{/each}
		{#if data.tags?.length > 0 && navigationDirectories[$directoryStatus.rootIndex[0]]?.pages?.[$directoryStatus.rootIndex[1]]?.queryTypes?.length > 0}
			<FoldedTags tagSet={data.tags} urlRedirect={data.slug} tagsOpen={!(data?.characters && !data?.commissionData?.characters) || expandedTags}/>
		{/if}
