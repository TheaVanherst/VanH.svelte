<script>
    ...
    export let
		data;
		data.artworks = // TODO: this needs putting into the api request
    ...
    let pagedQueryResults,
		queryData = queryFilter(data.artworks);

    onMount(() => {
        const
            initialSlug = $page.url.searchParams.get('gallery');
        	initialSlug ? galleryManager.galleryInitializing(data.artworks, initialSlug) : false; });
</script>

{#if !!data.artworks && data?.artworks?.length > 0}
	{#if queryData}
		<Pagination
				rows={queryData} perPage={10}
